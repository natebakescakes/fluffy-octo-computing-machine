...
def customer_contract_details(master_files, path):
    ...
    def customer_contract_details_ttc_contract_2(cell_row, cell_col, new_mod):
        ...
        try:
            if module_shipping_frequency[master_files['xl_sheet_main'].cell_value(cell_row, cell_col-2)] == shipping_calendar[ttc_contract_route[master_files['xl_sheet_main'].cell_value(cell_row, cell_col)]]:
                print ('TTC Contract No. check 2 --- Pass (%s)' % master_files['xl_sheet_main'].cell_value(cell_row, cell_col))
                update_df(new_mod, columns[cell_col], cell_row, PRIMARY_KEY_1, PRIMARY_KEY_2, 'PASS', shipping_calendar[ttc_contract_route[master_files['xl_sheet_main'].cell_value(cell_row, cell_col)]], module_shipping_frequency[master_files['xl_sheet_main'].cell_value(cell_row, cell_col-2)], 'Module Group shipping frequency match with shipping frequency of last ETD of Shipping Route')
            else:
                print ('TTC Contract No. check 2 --- Fail (Shipping Frequencies do not match)')
                update_df(new_mod, columns[cell_col], cell_row, PRIMARY_KEY_1, PRIMARY_KEY_2, 'WARNING', shipping_calendar[ttc_contract_route[master_files['xl_sheet_main'].cell_value(cell_row, cell_col)]], module_shipping_frequency[master_files['xl_sheet_main'].cell_value(cell_row, cell_col-2)], 'Module Group shipping frequency does not match with shipping frequency of last ETD of Shipping Route, please check if date overlaps with other ETDs')
        except KeyError:
            print ('TTC Contract No. check 2 --- Fail (Module Group / TTC Contract not found)')
            update_df(new_mod, columns[cell_col], cell_row, PRIMARY_KEY_1, PRIMARY_KEY_2, 'FAIL', master_files['xl_sheet_main'].cell_value(cell_row, cell_col), 'NA', 'Module Group Code / TTC Contract No. not found in system or submited masters')
