...
def supplier_parts(master_files, path):
    ...
    columns = {
        ...
        10: "Big Parts",
        11: "Non Order Matrix",
        12: "Supplier Box Length",
        13: "Supplier Box Width",
        14: "Supplier Box Height",
        15: "Supplier Box M3",
        16: "Supplier Area 1",
        17: "Supplier Area 2"
    }
    ...
    def check_maximum_length(cell_row, new_mod):
        ...
        working_columns = list(range(2, 18))
    ...
    def check_compulsory_fields(cell_row, new_mod):
        ...
        compulsory_fields = [2, 3, 4, 6, 10, 11, 12, 13, 14, 15]
        ...
        if part_and_supplier_code not in comparison_list_1:
            ...
            for row in range(9, selected['backup_3'].sheet_by_index(0).nrows):
                if part_and_supplier_code == str(selected['backup_3'].sheet_by_index(0).cell_value(row, 2)) + selected['backup_3'].sheet_by_index(0).cell_value(row, 3):
                    for col in range(0, 18): # Hard Code column range
    ...
    def supplier_parts_exp_country(cell_row, cell_col, new_mod):
        if master_files['xl_sheet_main'].cell_value(cell_row, cell_col) in region_master:
            if master_files['xl_sheet_main'].cell_value(cell_row, cell_col-4)[:2].find(master_files['xl_sheet_main'].cell_value(cell_row, cell_col)) != -1:
                print ('Exp Country check --- Pass (%s)' % master_files['xl_sheet_main'].cell_value(cell_row, cell_col))
                update_df(new_mod, columns[cell_col], cell_row, PRIMARY_KEY_1, PRIMARY_KEY_2, 'PASS', master_files['xl_sheet_main'].cell_value(cell_row, cell_col), master_files['xl_sheet_main'].cell_value(cell_row, cell_col-4), 'In Region Master and matches with Supplier Code')
            elif (any(master_files['xl_sheet_main'].cell_value(cell_row, cell_col) == x for x in ('C1', 'C2', 'C3')) and master_files['xl_sheet_main'].cell_value(cell_row, cell_col-4)[:2] == 'CN') or (any(master_files['xl_sheet_main'].cell_value(cell_row, cell_col) == x for x in ('I1', 'I2', 'I3')) and master_files['xl_sheet_main'].cell_value(cell_row, cell_col-4)[:2] == 'IN'):
                print ('Exp Country check --- Pass (%s)' % master_files['xl_sheet_main'].cell_value(cell_row, cell_col))
                update_df(new_mod, columns[cell_col], cell_row, PRIMARY_KEY_1, PRIMARY_KEY_2, 'PASS', master_files['xl_sheet_main'].cell_value(cell_row, cell_col), master_files['xl_sheet_main'].cell_value(cell_row, cell_col-4), 'In Region Master and matches with Supplier Code')
            else:
                print ('Exp Country check --- Fail (%s does not match with Supplier Code)' % master_files['xl_sheet_main'].cell_value(cell_row, cell_col))
                update_df(new_mod, columns[cell_col], cell_row, PRIMARY_KEY_1, PRIMARY_KEY_2, 'FAIL', master_files['xl_sheet_main'].cell_value(cell_row, cell_col), master_files['xl_sheet_main'].cell_value(cell_row, cell_col-4), 'Does not match with Supplier Code')
        else:
    ...
    def supplier_parts_west_part_master(cell_row, cell_col, new_mod):
        ...
        try:
            export_office_code = west_export[master_files['xl_sheet_main'].cell_value(cell_row, cell_col+5)]['Office Code']
        except KeyError:
    ...
    if len(listdir(path + '\\2) Backup')) == 0:
        ...
        for row in range(9, master_files['xl_sheet_main'].nrows):
            ...
            if str(master_files['xl_sheet_main'].cell_value(row, 0)).strip(' ') == 'NEW':
                ...
                supplier_parts_exp_country(row, 7, 'NEW')
                supplier_parts_next_srbq(row, 9, 'NEW')
                supplier_parts_big_parts(row, 10, 'NEW')
                supplier_parts_box_specs(row, 12, 'NEW')
                supplier_parts_box_m3(row, 15, 'NEW')
                ...
                if len(cols_to_check) != 0:
                    ...
                    if supplier_parts_mod_reference(row):
                        ...
                        for col in cols_to_check:
                            ...
                            if col+2 == 5:
                                supplier_parts_back_no(row, 5, 'MOD')
                            # Mod: Exp Country
                            if col+2 == 7:
                                supplier_parts_exp_country(row, 7, 'MOD')
                            # Mod: Next_SRBQ
                            if col+2 == 9:
                                supplier_parts_next_srbq(row, 9, 'MOD')
                            # Mod: Big Parts, Non Order Matrix
                            if (any(col+2 == x for x in (10, 11))):
                                supplier_parts_big_parts(row, 10, 'MOD')
                            # Mod: Supplier Box LWH
                            if (any(col+2 == x for x in (12, 13, 14))):
                                supplier_parts_box_specs(row, 12, 'MOD')
                            # Mod: Supplier Box M3
                            if col+2 == 15:
                                supplier_parts_box_m3(row, 15, 'MOD')
                            # Mod: optional columns
                            if (any(col+2 == x for x in (4, 6, 16, 17))):
