...
def customer_parts(master_files, path):
    ...
    def customer_parts_west_invoice(cell_row, cell_col, new_mod):
        if master_files['xl_sheet_main'].cell_value(cell_row, cell_col-2) in west_import.keys():
    ...
    def customer_parts_discontinued(cell_row, cell_col):
        ...
        try:
            for row in range(9, additional['TNM_IMP_CUSTOMER_CONTRACT_DETAI'].nrows):
                if part_no_customer_code == str(additional['TNM_IMP_CUSTOMER_CONTRACT_DETAI'].cell_value(row, 3)) + additional['TNM_IMP_CUSTOMER_CONTRACT_DETAI'].cell_value(row, 4):
                    discontinue_list.append((
